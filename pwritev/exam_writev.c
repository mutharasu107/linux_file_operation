...
/*                    it in a new file using pwritev() Also print the file position                             */
/****************************************************************************************************************/
#include <unistd.h>
#include <sys/types.h>
#include <sys/stat.h>
...
int main(int argc, char *argv[])
{
    ...
    if(argc < 7)
    {
       printf("Please atteached 5 different text file\n");
       exit(-15);
    }
    ...
    // int cre_fd = creat("fd.text", O_RDWR);
    int  c_fd = open(argv[6], O_CREAT | O_RDWR, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
    if(c_fd < 0 )
    {
    ...
    ssize_t pwrite_v = pwritev( c_fd, gather_arr, 5, 0 );
    if( pwrite_v < 0 )
    {
       perror("pwritev: Error write in to file\n");
       exit(-11);
       goto close_fd;
    }
